...
- `insert_overwrite`: If `partition_by` is specified, overwrite partitions in the table with new data. [Be sure to re-select _all_ of the relevant data for a partition. For Delta tables, use `partitions` to specify which ones should be replaced. Delta tables do not support dynamic partition overwrite.] If no `partition_by` is specified, overwrite the entire table with new data.  [Cannot be used when connecting via Databricks SQL Endpoints. For dynamic partition replacement with `method: odbc` + Databricks `cluster`, you must you **must** include `set spark.sql.sources.partitionOverwriteMode DYNAMIC` in the [cluster SparkConfig](https://docs.databricks.com/clusters/configure.html#spark-config). For atomic replacement of Delta tables, use the `table` materialization instead.]
