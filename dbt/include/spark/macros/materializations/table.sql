{% materialization table, adapter = 'spark', supported_languages=['sql', 'python', 'scala'] %}
...
{% macro scala_write_table(compiled_code, target_relation) %}
{{ compiled_code }}
// --- Autogenerated dbt materialization code. --- //
val dbt = new dbtObj(spark.table)
val df = model(dbt, spark)

// convert to org.apache.spark.sql.DataFrame if needed
if (df.isInstanceOf[DataFrame]) {
    // nothing to do here, already a DataFrame
} else {
    val msg = s"${df.getClass.getSimpleName} is not a supported type for dbt Scala materialization"
...
// how to execute scala model in notebook
// val dbt = new dbtObj(spark.table)
// val df = model(dbt, spark)
{%endmacro%}
